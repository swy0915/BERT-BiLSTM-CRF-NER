...
if os.name == 'nt':
    bert_path = r'E:\ai\1-workspace\1-prostate_segmentation\work_py\BERT-BiLSTM-CRF-NER-master\models\chinese_L-12_H-768_A-12'
    root_path = r'E:\ai\1-workspace\1-prostate_segmentation\work_py\BERT-BiLSTM-CRF-NER-master'
else:
    bert_path = '/home/macan/ml/data/chinese_L-12_H-768_A-12/'
    root_path = '/home/macan/ml/workspace/BERT-BiLSTM-CRF-NER'

flags.DEFINE_string(
    "data_dir", os.path.join(root_path, 'hainan_data'),
    "The input datadir.",
)
...
flags.DEFINE_string(
    "task_name", 'hn', "The name of the task to train."
)

flags.DEFINE_string(
    "output_dir", os.path.join(root_path, 'output_hn\\result_dir'),
...
flags.DEFINE_string(
    ...
    # "init_checkpoint", os.path.join(bert_path, 'model.ckpt-100'),
...
flags.DEFINE_float("num_train_epochs", 30.0, "Total number of training epochs to perform.")
    ...
    def _read_data(cls, input_file):
        ...
        with codecs.open(input_file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                contends = line.strip()
                split_str = '\t'
                tokens = contends.split(split_str)
                if len(tokens) == 2:
                    word = line.strip().split(split_str)[0]
                    label = line.strip().split(split_str)[-1]
                else:
                    if len(contends) == 0:
...
class HnProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
    ...
    def get_labels(self):
        return ["O","B-PER","I-PER","B-SCE","I-SCE","B-TIM","I-TIM",
                "B-HOT","I-HOT","B-RES","I-RES","B-DLO","I-DLO",
                "B-DIS","I-DIS","B-ORG","I-ORG","B-TEL","I-TEL",
                "B-PRI","I-PRI","B-TIC","I-TIC","X","[CLS]","[SEP]"]
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    with tf.variable_scope("loss"):
        ...
        logits = tf.reshape(logits, [-1, FLAGS.max_seq_length, num_labels])  # 27 is the labels_num
        log_probs = tf.nn.log_softmax(logits, axis=-1)
        one_hot_labels = tf.one_hot(labels, depth=num_labels,
                                    dtype=tf.float32)  # enlarge dim convert label_id to an one-hot vector
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            train_op = optimization.create_optimizer(
                # return an operation, it implement gradient calculation and parameter update
                total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu)
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                # TPUEstimatorSpec is a simple structure for naming fields,contains all tf.Tensors.prepared for TPUEstimators
                mode=mode,
                loss=total_loss,
                train_op=train_op,
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        # 打印验证集数据信息
        num_train_size = data_config.get('num_train_size', 0)
        tf.logging.info("***** Running train *****")
        tf.logging.info("  Num examples = %d", num_train_size)
        tf.logging.info("  Batch size = %d", FLAGS.train_batch_size)
        train_steps = None
        if FLAGS.use_tpu:
            train_steps = int(num_train_size / FLAGS.train_batch_size)
        ...
        result = estimator.evaluate(input_fn=train_input_fn, steps=train_steps)
        output_train_file = os.path.join(FLAGS.output_dir, "train_results.txt")
    ...
    if FLAGS.do_predict:
        ...
        with codecs.open(os.path.join(FLAGS.output_dir, 'entity_level_predicted_result.txt'), 'w',
                         encoding='utf-8') as fd:
            fd.write(''.join(eval_result))


def load_data():
    processer = HnProcessor()

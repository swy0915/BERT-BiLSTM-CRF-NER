...
flags.DEFINE_string(
    "output_dir", os.path.join(root_path, 'output_hn_crf\\result_dir'),
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, logits, trans):
                # 首先对结果进行维特比解码
                # crf 解码
                label_tags = list(range(2, num_labels-3))
                weight = tf.sequence_mask(FLAGS.max_seq_length)
                precision = tf_metrics.precision(label_ids, pred_ids, num_labels, label_tags, weight)
                recall = tf_metrics.recall(label_ids, pred_ids, num_labels, label_tags, weight)
                f = tf_metrics.f1(label_ids, pred_ids, num_labels, label_tags, weight)
